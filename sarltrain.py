...
record_loss = np.zeros([n_episode, 1])
...
for i_episode in range(n_episode):
    ...
    for i_step in range(n_step_per_episode):
        ...
        action = agent.predict(state, epsi)
        ...
        if time_step % mini_batch_step == mini_batch_step - 1:
            # q-learning
            loss_val_batch = agent.Q_Learning_mini_batch()
            record_loss[i_episode] = loss_val_batch
            if i_episode % 50 == 0:
                print('episode:', i_episode, 'loss:', loss_val_batch)
...
current_dir = os.path.dirname(os.path.realpath(__file__))
reward_path = os.path.join(current_dir, 'model/'+ label + '/reward.mat')  # "model/" + label + '/reward.mat')
